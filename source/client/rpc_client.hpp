...
namespace bitrpc
{
    namespace client
    {
        class RegistryClient // 封装服务注册客户端
        {
        public:
        ...
        private:
            ...
            // 通过查找指定服务的提供者地址信息，查看是否存在已实例化的客户端，没有则创建
        ...
        private:
            ...
            bool _enableDiscovery;                  // _enableDiscovery--是否启用服务发现功能，也决定了传入的地址信息是注册中心的地址，还是服务提供者的地址
            DiscoveryClient::ptr _discovery_client; // 基于服务发现的客户端
