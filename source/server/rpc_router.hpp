...
namespace bitrpc {
    namespace server {              //再加上server命名空间，客户端与服务端部分代码功能类似，避免命名冲突

        enum class VType {          //枚举类        
            BOOL = 0,               //枚举出rpc请求参数类型
            INTEGRAL,
            NUMERIC,
            STRING,
        ...
        class RpcRouter {          //对外RpcRouter类，两点功能：1.注册服务，2.处理rpc请求
            public:
