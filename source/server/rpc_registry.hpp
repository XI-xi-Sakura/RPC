...
namespace bitrpc
{
    namespace server
    {
        class ProviderManager // 管理两个方面：各个服务可由哪些主机提供以及连接与服务提供者的关系
        {
        public:
            ...
            // 增加服务提供者，具体参数为服务提供者连接，服务提供者地址，服务名称
            ...
            void delProvider(const BaseConnection::ptr &c)
            {
                ...
                {
                    auto &providers = _providers[method]; // std::unordered_map<std::string, std::set<Provider::ptr>> _providers;
        ...
        class DiscovererManager // 管理两个方面：各个服务被哪些主机所关注（发现）以及连接与服务关注（发现）者的关系
        {
        public:
        ...
        class PDManager // 整合ProviderManager和DiscovererManager
        {
        public:
